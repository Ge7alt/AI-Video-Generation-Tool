...
if __name__ == '__main__': 
    ...
    parser.add_argument('--driving_text', default='./data/input/Text Data/6.txt', help="path to driving text")
    ...
    text_name = os.path.split(opt.driving_text)[1][:-4]
    save_root = join('./results/', opt.id, text_name)
    ...
    audio_save = join(save_root, text_name + '.mp3')
    ################################# tts #####################################
    tts_audio = nepali_tts(opt.driving_text, audio_save)

    
    ############################## Inference ##################################
    print('Processing audio: {} ...'.format(text_name)) 
    # read audio
    audio, _ = librosa.load(tts_audio, sr=sr)
    ...
    final_path = join(save_root, text_name + '.avi')
    write_video_with_audio(tmp_audio_path, final_path, 'pred_')
    feature_maps_path = join(save_root, text_name + '_feature_maps.avi')
